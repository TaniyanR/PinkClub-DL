...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    if (!rate_limit_allow('password_reset', 3, 900)) {
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            $registeredEmail = strtolower(trim(setting_admin_email('')));
            if ($registeredEmail !== '' && hash_equals($registeredEmail, $email)) {
                ...
                if (is_array($admin) && db_table_exists('admin_password_resets')) {
                    ...
                    try {
                        ...
                        error_log('password reset token creation failed');
                    }
                    ...
                    if ($tokenStored) {
                        ...
                        $siteName = trim(site_setting_get('site.title', site_setting_get('site.name', 'PinkClub-DL')));
                        if ($siteName === '') {
                            $siteName = 'PinkClub-DL';
                        }
                        $safeSiteName = preg_replace('/[\r\n]+/', ' ', $siteName) ?: 'PinkClub-DL';
                        $subject = '[' . $safeSiteName . '] パスワード再設定';
                        $encodedSubject = function_exists('mb_encode_mimeheader')
                            ? mb_encode_mimeheader($subject, 'UTF-8')
                            : $subject;
                        $headers = 'From: ' . $safeSiteName . ' <' . $fromEmailForLog . ">\r\nContent-Type: text/plain; charset=UTF-8";
                        $mailAttempted = true;
                        $mailBodyForLog = $body;
