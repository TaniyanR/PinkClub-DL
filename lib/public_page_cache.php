...
function pcf_public_page_cache_start(int $ttlSeconds=120): void
{
    ...
    $cacheGeneration=$scriptName==='item.php'?'v20-social-card':'v19';
    $key=hash('sha256',$cacheGeneration.'|'.pcf_public_page_cache_authority().'|'.(pcf_public_request_is_mobile()?'sp':'pc').'|'.$normalizedUri);$file=$dir.'/'.$key.'.html';$lockFile=$dir.'/.regenerate-'.substr($key,0,1).'.lock';
